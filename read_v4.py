...
directory = 'C:\\Users\\joseb\\Documents\\Projects\\kyp_earthquakes\\eq_data\\SCEC_DC'
...
# 	location_ = [Point(xy) for xy in zip(lon[], lon[i])]
...
for i in range(len(title)): 
    #size = mag[i]**2
    for j in range(len(title)):
    	location_ = [Point(xy) for xy in zip(lon[i][j], lon[i][j])]
    	location = GeoDataFrame(location, geometry=location_)
...
for i in range(len(title)):
	...
	location_df.index.name = str(1932 + i)
